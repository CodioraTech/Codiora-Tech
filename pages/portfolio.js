            ...
            <section className="min-h-screen bg-gray-50 dark:bg-dark pt-32 pb-20 relative transition-colors duration-300">
                ...
                <div className="container mx-auto px-6 relative z-10">
                    ...
                    <div className="text-center mb-16">
                        ...
                        >
                            <span className="text-gray-900 dark:text-white transition-colors">Selected </span>
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-secondary">Works</span>
                        </motion.h1>
                        <p className="text-xl text-gray-600 dark:text-gray-400 transition-colors">Digital experiences that set new standards.</p>
                    </div>
                        ...
                        {["All", "Solutions", "AI", "Labs"].map((cat) => (
                            <button
                                ...
                                className={`px-8 py-2 rounded-full border transition-all duration-300 ${filter === cat
                                    ...
                                    : 'bg-transparent border-gray-300 dark:border-white/10 text-gray-600 dark:text-gray-400 hover:border-gray-400 dark:hover:border-white/50 hover:text-gray-900 dark:hover:text-white'
                                    }`}
                            >
                                {cat}
                            </button>
                            ...
                            {filteredProjects.map((project) => (
                                <Link href={`/portfolio/${project.slug}`} key={project.id}>
                                    <motion.div
                                        ...
                                        whileHover={{ y: -10 }}
                                        className="group relative h-80 bg-white dark:bg-gray-900 rounded-2xl border border-black/5 dark:border-white/5 overflow-hidden cursor-pointer shadow-lg dark:shadow-none transition-colors"
                                        style={{ perspective: 1000 }}
                                    >
                                        {/* Background Image */}
                                        <div className="absolute inset-0 transition-transform duration-700 group-hover:scale-110">
                                            {/* Fallback & Image Container */}
                                            <div className="relative w-full h-full bg-gradient-to-br from-gray-100 to-white dark:from-gray-800 dark:to-black">
                                                <img
                                                    src={project.image}
                                                    alt={project.title}
                                        ...
                                        <div className="absolute inset-0 flex flex-col justify-end p-8 bg-gradient-to-t from-gray-900 via-gray-900/40 to-transparent transition-colors z-20">
                                            <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                                ...
                                                <p className="text-gray-300 text-sm mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">{project.description}</p>

                                                <div className="flex items-center text-white text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-200">

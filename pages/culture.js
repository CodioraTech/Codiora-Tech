...
export default function Culture() {
    ...
    return (
        <>
            ...
            <div ref={containerRef} className="bg-[#020202] min-h-screen text-white overflow-hidden selection:bg-pink-500/30">
                ...
                <div className="container mx-auto px-6 pt-32 pb-40 relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-40">
                        {values.map((v, i) => (
                            <motion.div
                                ...
                                className="bg-[#0a0a0a] border border-white/5 p-16 rounded-[3rem] hover:bg-white/5 transition-colors group"
                            >
                                <div className="text-pink-500 text-sm font-bold uppercase tracking-widest mb-6">0{i + 1}</div>
                                <h3 className="text-4xl font-bold text-white mb-6 group-hover:text-pink-400 transition-colors">{v.title}</h3>
                                <p className="text-gray-400 text-lg leading-relaxed max-w-md">{v.desc}</p>
                            </motion.div>
                        ))}
                    </div>

                    {/* Visual Section (Video Player) */}
                    <div className="relative rounded-[3rem] overflow-hidden aspect-video mb-40 border border-white/10 group cursor-pointer bg-black" onClick={() => setIsPlaying(true)}>
                        {isPlaying ? (
                            <iframe
                                width="100%"
                                height="100%"
                            ...
                            <>
                                ...
                                <div className="absolute bottom-10 left-10 text-white pointer-events-none">
                                    ...
                                    <p className="text-gray-300">Watch how we build the future.</p>
                                </div>
                            </>
                        )}
                    </div>
                    ...
                    {/* Stats */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 border-t border-white/10 pt-20">
                        {[
                            ...
                            <div key={i} className="text-center">
                                ...
                                <div className="text-gray-500 text-xs font-bold uppercase tracking-widest">{stat.lbl}</div>
                            </div>
                        ))}
                    </div>
                </div>
